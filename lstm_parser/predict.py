...
def predict(options):
    ...
    model = ParserModel(network_params.params, embeddings=embeddings, model_path=options['parser_model_load_path'])
    ...
    for data_set_name in data_set:
        ...
        for sent_idx, sentence in enumerate(data):
            _, eval_dict = predict_and_get_evaluation(sentence, model, reverse_act_map)
